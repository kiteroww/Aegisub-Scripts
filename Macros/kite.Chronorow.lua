...
function join_same_text(subs,sel) table.sort(sel,function(a,b)return a>b end) for _,i in ipairs(sel)do if subs[i] then local l=subs[i] if i>1 and subs[i-1] and subs[i-1].text==l.text then local p=subs[i-1] p.end_time=math.max(p.end_time,l.end_time) subs[i-1]=p subs.delete(i) end end end end
function time_picker(subs, sel)
    if #sel == 0 then
        aegisub.dialog.display({{class="label", label="Error: No lines selected."}}, {"OK"})
        return sel
    end
    ...
    if min_start == nil or max_end == nil then
        aegisub.dialog.display({{class="label", label="Error: No valid active dialogue lines found."}}, {"OK"})
        return sel
    end
...
function style_sentinel(subs, selected_lines)
    local styles_found = {}
    local styles_list = ""
    ...
    if styles_list == "" then
        aegisub.dialog.display({{class="label", label="No styles found in selected lines."}}, {"OK"})
        return
    end
...
local hd_info={
    ...
    ["Time Picker"]="Selects active dialogue lines within time range (excludes comments)",
...
• Time Picker - Selects active dialogue lines by time range (excludes comments)
• Caption Clarifier - Removes all types of annotations with nested support
• Style Sentinel - Filters by styles with safety confirmations
    ...
    while true do
        ...
        elseif b=="EXECUTE" then
            ...
            if r.doggo_mode then
                ...
                elseif r.ht=="Time Picker" then 
                    local new_sel = time_picker(subs,tsel)
