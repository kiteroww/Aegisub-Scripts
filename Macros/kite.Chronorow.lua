...
local function sliceSentences(t,rx,keep) local o,pos={},1 while pos<=#t do local s,e=t:find(rx,pos) if s then local ch,stop=t:sub(s,s),e while t:sub(stop+1,stop+1)==ch do stop=stop+1 end if keep then while t:sub(stop+1,stop+1):match("%s") do stop=stop+1 end end o[#o+1]=t:sub(pos,stop) pos=stop+1 while t:sub(pos,pos):match("%s") do pos=pos+1 end else o[#o+1]=t:sub(pos) break end end return o end
