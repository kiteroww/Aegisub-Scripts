...
local LANG = {
    en = {
        ...
        cue_lbl_files      = "Data files (auto-detected, editable):",
        ...
        cue_hint           = "Multi-chapter ignores the path boxes (chapter files by layer). LZT modes ignore the ms tuning (Config).",
    ...
    es = {
        ...
        cue_lbl_files      = "Archivos de datos (autodetectados, editables):",
        ...
        cue_hint           = "Multi-capítulo ignora las casillas de ruta (archivos por layer). Los modos LZT ignoran los ajustes en ms (Config).",
    ...
    pt = {
        ...
        cue_lbl_files      = "Arquivos de dados (autodetectados, editáveis):",
        ...
        cue_hint           = "Multi-capítulo ignora as caixas de caminho (arquivos por camada). Os modos LZT ignoram os ajustes em ms (Config).",
...
local EXTRA_LANG = {
    en = {
        ...
        cfg_lbl_leadutil="Add Lead-In / Lead-Out / Chain", cfg_lbl_lead_step="Step (ms):", cfg_lbl_chain_cap="Chain max (ms):", cfg_chk_table_csv="Write noise_table CSV (Table)",
        ...
        msg_edge_snap_done="Directional snapping done.\n\nLines modified: %d\nRange: %d ms",
    ...
    es = {
        ...
        cfg_lbl_leadutil="Añadir Lead-In / Lead-Out / Cadena", cfg_lbl_lead_step="Paso (ms):", cfg_lbl_chain_cap="Cadena máx (ms):", cfg_chk_table_csv="Escribir CSV noise_table (Table)",
        ...
        msg_edge_snap_done="Snapping direccional finalizado.\n\nLineas modificadas: %d\nRango: %d ms",
    ...
    pt = {
        ...
        cfg_lbl_leadutil="Adicionar Lead-In / Lead-Out / Cadeia", cfg_lbl_lead_step="Passo (ms):", cfg_lbl_chain_cap="Cadeia máx (ms):", cfg_chk_table_csv="Gravar CSV noise_table (Table)",
        ...
        msg_edge_snap_done="Snapping direcional concluido.\n\nLinhas modificadas: %d\nAlcance: %d ms",
...
local LOCALE_PATCH = {
    en = {
        ...
        msg_actors_updated       = "Updated %d actors in %d lines.",
        ...
        lbl_new_keep_empty       = "New (empty = keep)",
    ...
    es = {
        ...
        msg_actors_updated       = "Se actualizaron %d actores en %d líneas.",
        ...
        lbl_original             = "Original",
        lbl_new_keep_empty       = "Nuevo (vacío = conservar)",
    ...
    pt = {
        ...
        msg_actors_updated       = "Atualizados %d atores em %d linhas.",
        ...
        lbl_original             = "Original",
        lbl_new_keep_empty       = "Novo (vazio = manter)",
        lbl_original_text        = "TEXTO ORIGINAL",
...
local UI = {
    labels = {
        en = {
            ...
            ["Ends Only"]="Ends only", ["Start Only"]="Start only", ["Full Audit"]="Full audit", ["Duration"]="Duration", ["CPS"]="CPS", ["Gaps"]="Gaps", ["Overtime"]="Overtime",
            ...
            ["Split by Comma"]="Split by comma", ["Join Lines"]="Join lines", ["Join Same Text"]="Join same text", ["Divide by \\N"]="Divide by \\N",
            ...
            ["Add Lead-In"]="Add lead-in", ["Add Lead-Out"]="Add lead-out", ["Chain Left"]="Chain left", ["Chain Right"]="Chain right",
            ["Shift First"]="Shift first", ["Romaji Karaoker (Word → \\k)"]="Romaji karaoker (word → \\k)",
            ["AE Export"]="AE export", ["Actor Manager"]="Actor manager", ["Text Replacer"]="Text replacer", ["mpv QC"]="mpv QC", ["Style Filter"]="Style filter",
        },
        es = {
            ...
            ["Ends Only"]="Solo finales", ["Start Only"]="Solo inicios", ["Full Audit"]="Auditoría completa", ["Duration"]="Duración", ["CPS"]="CPS", ["Gaps"]="Gaps", ["Overtime"]="Sobretiempo",
            ...
            ["Split by Comma"]="Dividir por coma", ["Join Lines"]="Unir líneas", ["Join Same Text"]="Unir texto igual", ["Divide by \\N"]="Dividir por \\N",
            ...
            ["Add Lead-In"]="Añadir lead-in", ["Add Lead-Out"]="Añadir lead-out", ["Chain Left"]="Encadenar izquierda", ["Chain Right"]="Encadenar derecha",
            ["Shift First"]="Desplazar desde primera", ["Romaji Karaoker (Word → \\k)"]="Karaoke romaji (palabra → \\k)",
            ["AE Export"]="Exportar AE", ["Actor Manager"]="Gestor de actores", ["Text Replacer"]="Reemplazar texto", ["mpv QC"]="mpv QC", ["Style Filter"]="Filtro de estilo",
        },
        pt = {
            ...
            ["Ends Only"]="Só finais", ["Start Only"]="Só inícios", ["Full Audit"]="Auditoria completa", ["Duration"]="Duração", ["CPS"]="CPS", ["Gaps"]="Gaps", ["Overtime"]="Sobretempo",
            ...
            ["Split by Comma"]="Dividir por vírgula", ["Join Lines"]="Unir linhas", ["Join Same Text"]="Unir texto igual", ["Divide by \\N"]="Dividir por \\N",
            ...
            ["Add Lead-In"]="Adicionar lead-in", ["Add Lead-Out"]="Adicionar lead-out", ["Chain Left"]="Encadear esquerda", ["Chain Right"]="Encadear direita",
            ["Shift First"]="Deslocar desde primeira", ["Romaji Karaoker (Word → \\k)"]="Karaokê romaji (palavra → \\k)",
            ["AE Export"]="Exportar AE", ["Actor Manager"]="Gestor de atores", ["Text Replacer"]="Substituir texto", ["mpv QC"]="mpv QC", ["Style Filter"]="Filtro de estilo",
        },
    },
}
...
and writes its markers to the Effect field.
   ...
   3.6. Gaps: checks line-to-line gaps against the Short gap and Large
        gap thresholds. Markers are written on both adjacent lines and
        include the detected millisecond value.
   3.7. Overtime: marks lines that exceed the configured maximum
        duration.
...
7.6. TIME / SORT
    ...
    - Add Lead-In: moves the start back by the configured step
      (Config → Add Lead-In / Lead-Out). A neighbour chained at gap 0
      moves its end along to keep the chain; with a positive gap the
      start only advances until it touches the neighbour, never
      overlapping it.
    - Add Lead-Out: the same for the end, pushing the start of a
      chained next line.
...
--------------
   ...
   9.2. Actor Manager: renames or merges actors within the selection.
   9.3. Text Replacer: replaces visible text while preserving override
        tags.
   9.4. mpv QC: reads notes exported from mpvQC in the format
   ...
   9.5. Remover Assistant: removes selected visible signs, spacing
        tokens, comments, and known override tags without toggling them
        back on when absent.
   9.6. Style Filter: filters or deletes lines by style with a
        confirmation dialog.
...
the main Chrono Suite configuration.
   ...
   10.1. Modes:
         ...
         - LZT Raw Silences: uses only the silence files, without flux
           or VAD.
   10.2. Data files are auto-detected in the subtitle folder using
         the ProcesarLote.bat naming (NN_Retimes_30/40/50.txt,
...
Global configuration persisted between sessions.
   ...
   14.4. Add Lead-In / Lead-Out / Chain: step in milliseconds for the
   ...
   14.7. Audit Presets: Twin/Miss pairs for Start, End, and Full;
         Short/Long thresholds; maximum CPS; short and large gaps;
         overtime; maximum width.
...
auditoría e inscribe sus marcadores en el campo Effect.
   ...
   3.6. Gaps: revisa los espacios entre líneas contra los umbrales
        Gap corto y Gap largo. Los marcadores se inscriben en ambas
        líneas adyacentes con los milisegundos detectados.
   3.7. Sobretiempo: marca las líneas que superan la duración máxima
        configurada.
...
7.6. TIEMPO / ORDEN
    ...
    - Add Lead-In: adelanta el inicio el paso configurado (Config →
      Añadir Lead-In / Lead-Out). Si hay una vecina encadenada con
      gap 0, su final se mueve junto con el inicio para mantener la
      cadena; si el gap es positivo, el inicio solo avanza hasta
      tocar a la vecina (gap 0), nunca la solapa.
    - Add Lead-Out: lo mismo para el final, empujando el inicio de la
      siguiente línea si está encadenada.
...
---------------------
   ...
   9.1. AE Export: exporta datos de movimiento compatibles con After
        Effects.
   9.2. Actor Manager: renombra o fusiona actores en la selección.
   9.3. Text Replacer: reemplaza texto visible conservando los
        override tags.
   9.4. mpv QC: lee notas exportadas desde mpvQC con formato
        ...
        en milisegundos.
   9.5. Remover Assistant: elimina signos visibles, espacios,
        comentarios y override tags conocidos sin alternarlos si no
        existen.
   9.6. Style Filter: filtra o elimina líneas por estilo con diálogo
        de confirmación.
...
numéricos se guardan en la configuración principal de Chrono Suite.
   ...
   10.1. Modos:
         ...
         - LZT Bruto Silencios: usa solo los archivos de silencios,
           sin flux ni VAD.
   10.2. Los archivos de datos se autodetectan en la carpeta del
         subtítulo con la nomenclatura de ProcesarLote.bat
         (NN_Retimes_30/40/50.txt, NN_Retimes_vad.tsv,
...
Configuración global persistente entre sesiones.
   ...
   14.4. Añadir Lead-In / Lead-Out / Cadena: paso en milisegundos de
   ...
   14.7. Presets de auditoría: pares Twin/Miss para Inicio, Final y
         Completa; umbrales Corto/Largo; CPS máxima; gaps cortos y
         largos; sobretiempo; ancho máximo.
...
auditoria e inscreve seus marcadores no campo Effect.
   ...
   3.5. CPS: verifica a velocidade de leitura contra o máximo
        configurado.
   3.6. Gaps: verifica os espaços entre linhas contra os limites
        Gap curto e Gap longo. Os marcadores são inscritos em ambas
        as linhas adjacentes com os milissegundos detectados.
   3.7. Sobretempo: marca as linhas que ultrapassam a duração máxima
        configurada.
...
7.6. TEMPO / ORDEM
    ...
    - Add Lead-In: antecipa o início pelo passo configurado (Config →
      Adicionar Lead-In / Lead-Out). Se houver uma vizinha encadeada
      com gap 0, o fim dela se move junto para manter a cadeia; com
      gap positivo, o início só avança até tocar a vizinha (gap 0),
      nunca a sobrepõe.
    - Add Lead-Out: o mesmo para o fim, empurrando o início da linha
      seguinte se estiver encadeada.
...
---------------------
   ...
   9.1. AE Export: exporta dados de movimento compatíveis com o After
        Effects.
   9.2. Actor Manager: renomeia ou mescla atores na seleção.
   9.3. Text Replacer: substitui texto visível preservando os
        override tags.
   9.4. mpv QC: lê notas exportadas do mpvQC no formato
        ...
        milissegundos.
   9.5. Remover Assistant: remove sinais visíveis, espaços,
        comentários e override tags conhecidos sem alterná-los quando
        ausentes.
   9.6. Style Filter: filtra ou apaga linhas por estilo com diálogo
        de confirmação.
...
numéricos são salvos na configuração principal do Chrono Suite.
   ...
   10.1. Modos:
         ...
         - LZT Bruto Silêncios: usa apenas os arquivos de silêncios,
           sem flux nem VAD.
   10.2. Os arquivos de dados são autodetectados na pasta da legenda
         com a nomenclatura do ProcesarLote.bat
         (NN_Retimes_30/40/50.txt, NN_Retimes_vad.tsv,
...
Configuração global persistente entre sessões.
   ...
   14.3. SCXvid: caminho do SCXvid, caminho do FFmpeg e sufixo do
         log.
   14.4. Adicionar Lead-In / Lead-Out / Cadeia: passo em
   ...
   14.7. Presets de auditoria: pares Twin/Miss para Início, Fim e
         Completa; limites Curto/Longo; CPS máximo; gaps curtos e
         longos; sobretempo; largura máxima.
...
local function directionalSnap(subs, sel, edge, direction)
    ...
    showMsg(string.format(L("msg_edge_snap_done"), modified, limit))
end
...
cueTimer = function(subs, sel)
    ...
    VT.discover_paths(VT.session_paths())
    ...
    while true do
        local chapters = cueDetectChapters(VT)
        ...
        local d = {
            ...
            { class="label", label=L("cue_lbl_files"), x=0, y=2, width=12, height=1 },
...
local function actorManager(subs, sel)
    local unique, seen = {}, {}
    for _, i in ipairs(sel) do
        local line = subs[i]
        if isDialogue(line) then
            local a = normalizeString(line.actor)
            if not seen[a] then seen[a] = true; table.insert(unique, a) end
        end
    end
    table.sort(unique)
    local original_text = table.concat(unique, "\n")
    local dlg = {
        { class="label",   label=L("lbl_original"),                 x=0,  y=0, width=15, height=1 },
        { class="label",   label=L("lbl_new_keep_empty"),            x=15, y=0, width=15, height=1 },
    ...
    local new_actors = parse_ml(res.dest)
    local map = {}
    ...
    showMsg(string.format(L("msg_actors_updated"), uc, changed))
end
...
local UTILITY_SECTIONS = {
    ...
    },
    {
        ...
        items = {
            ...
            { name="Add Lead-In",                       func = TimingOperations.addLeadIn, includeVector = true },
            { name="Add Lead-Out",                      func = TimingOperations.addLeadOut, includeVector = true },
...
local SUITE_TOOLS = {
    ...
    { name = "Actor Manager",       func = actorManager },
...
local function rowMasterGui(subs, sel)
    ...
    while true do
        local ui = {
            ...
            preset = UI.options({ "","Ends Only","Start Only","Full Audit","Duration","CPS","Gaps","Overtime" }, state.preset),
...
local macros = {
    ...
    { macroPath("Cue Timer"),          "Auto-time from detected voice and timing data", cueTimer },
    { macroPath("Extract KF (SCXvid)"),"Generate keyframes via SCXvid",     runScxvid },
    { macroPath("Scream Detector"),     "Mark loud dialogue intervals",      screamDetector },
...
for _, sec in ipairs(UTILITY_SECTIONS) do
    ...
    for _, entry in ipairs(sec.items) do
        if entry.name and entry.name ~= "" and entry.func then
            ...
            registerMacro(macroPath(root .. "/" .. entry.name), entry.name, makeUtilityMacro(entry))
        end
    end
end
...
for _, tool in ipairs(SUITE_TOOLS) do
    if tool.name and tool.name ~= "" and tool.func then
        registerMacro(macroPath("Tools/" .. tool.name), tool.name, makeSuiteToolMacro(tool))
    end
end
